...
<div class="container">
	...
	<div class="row">
		<div class="col-sm-12 home-main">
				<?php if($_GET['wish'] && (mc_get_page_field($_GET['group'],'type')=='pro' || mc_get_page_field($_GET['group'],'type')=='pro_pending')) : ?>
				<h4 class="title mb-0">
				<i class="icon-heart"></i> 写下您的合伙优势，让我们帮您一起实现梦想！
			</h4>
			<div class="wishbox mb-20">
					...
					<div class="col-sm-8">
						<div class="media">
							<a class="pull-left"
								href="<?php echo mc_get_url($_GET['group']); ?>"> <img
								width="100" src="<?php echo $fmimg_args[0]; ?>"
								alt="<?php echo mc_get_page_field($_GET['group'],'title'); ?>">
							</a>
							<div class="media-body">
								<h4 class="media-heading mb-20">
									我想要合伙做 <a href="<?php echo mc_get_url($_GET['group']); ?>"><?php echo mc_get_page_field($_GET['group'],'title'); ?></a>
								</h4>
									...
									<p class="mb-0">我合伙资金份数: <?php echo $cart; ?>份</p>
							</div>
						</div>
					</div>
					...
					?>
							我合伙出资：<span><?php echo sprintf("%.2f",($cart_price*$cart)/10000); ?></span>万
					</div>
				</div>
			</div>
				<?php endif; ?>
				<form role="form" method="post"
				action="<?php echo U('Home/perform/publish'); ?>">
					<?php if($_GET['wish'] && (mc_get_page_field($_GET['group'],'type')=='pro' || mc_get_page_field($_GET['group'],'type')=='pro_pending'))  : ?>
					<input type="hidden" name="number" value="<?php echo $cart; ?>">
					<?php if($parameter) : foreach($parameter as $key=>$valp) : ?>
					<input type="hidden" name="parameter[<?php echo $key; ?>]"
					value="<?php echo $valp; ?>">
					...
					<div class="col-sm-4 col-lg-3">
						<div class="form-group">
							<label> 版块 </label> <select class="form-control" name="group">
								<?php if(mc_get_page_field($_GET['group'],'type')=='pro' || mc_get_page_field($_GET['group'],'type')=='pro_pending') : ?>
								...
								<?php $group = M('page')->where('type="group"')->order('date desc')->select(); if($group) : foreach($group as $val) : ?>
									...
									<?php if($_GET['group']==$val['id']) echo 'selected'; ?>><?php echo $val['title']; ?></option>
								<?php endforeach; endif; ?>
								<?php endif; ?>
								</select>
						</div>
				...
				<div class="form-group">
					<label> 主题内容 </label>
					<textarea name="content" class="form-control" rows="3"><?php if($_GET['wish'] && mc_get_page_field($_GET['group'],'type')=='pro') : ?><h4
							class="mb-20">我为什么想要合伙这个项目</h4>
						<p>写下自己合伙这个项目的想法，写的越详细，项目就越容易实现哦。</p><?php endif; ?></textarea>
				</div>
					<?php if($_GET['wish'] && mc_get_page_field($_GET['group'],'type')=='pro') : ?>
					<div class="form-group">
					<label> 你的通讯信息 - 方便项目合作中联系。 </label>
					<div class="row">
						<div class="col-sm-4">
							<input type="text" name="buyer_name" class="form-control"
								placeholder="您的姓名"
								value="<?php echo mc_get_meta(mc_user_id(),'buyer_name',true,'user'); ?>">
						</div>
					</div>
				</div>
				<div class="form-group">
						...
						<div class="col-sm-4">
							<div class="input-group">
								<input type="text" class="form-control"
									value="<?php echo mc_get_meta(mc_user_id(),'buyer_province',true,'user').' '.mc_get_meta(mc_user_id(),'buyer_city',true,'user'); ?>"
									disabled> <span class="input-group-addon" id="repc"> <i
									class="glyphicon glyphicon-refresh"></i>
								</span> <input type="hidden" name="buyer_province"
									value="<?php echo mc_get_meta(mc_user_id(),'buyer_province',true,'user'); ?>">
								<input type="hidden" name="buyer_city"
									value="<?php echo mc_get_meta(mc_user_id(),'buyer_city',true,'user'); ?>">
							</div>
						</div>
					</div>
					<script>
						...
						<div class="col-sm-2">
							<select class="form-control" id="province" tabindex="4"
								...
								name="buyer_province" datatype="*" errormsg="必须选择您所在的地区"></select>
						</div>
						<div class="col-sm-2">
							<select class="form-control" id="city" tabindex="4"
								disabled="disabled" runat="server" name="buyer_city"></select>
						</div>
					</div>
				...
				<div class="form-group">
					<textarea class="form-control" name="buyer_address" rows="3"
						placeholder="区县、街道、门牌号"><?php echo mc_get_meta(mc_user_id(),'buyer_address',true,'user'); ?></textarea>
				</div>
				<div class="form-group">
					<div class="row">
						<div class="col-sm-4">
							<input type="text" class="form-control" name="buyer_phone"
								placeholder="联系电话，非常重要"
								value="<?php echo mc_get_meta(mc_user_id(),'buyer_phone',true,'user'); ?>">
						</div>
					</div>
				</div>
					<?php endif; ?>
