...
<div class="container">
	<ol class="breadcrumb mb-20 mt-20" id="baobei-term-breadcrumb">
		...
		<div class="pull-right">
			...
			<a href="<?php echo U('publish/index/add_group'); ?>">新建版块</a> <a
				href="javascript:;" class="publish"><?php echo M('page')->where("type = 'publish'")->count('id'); ?></a>
	...
	<div class="row">
		<div class="col-sm-8 col-lg-9" id="group">
				...
				<?php foreach($groups as $val) : ?>
					...
					href="<?php echo U('post/group/single?id='.$val['id']); ?>">
							<?php echo $val['title']; ?>
						</a></li>
				<?php endforeach; ?>
				</ul>
						...
						<div class="form-group">
							<label> 版块 </label> <select class="form-control" name="group">
								<?php $group = M('page')->where('type="group"')->order('date asc')->select(); if($group) : foreach($group as $val) : $num++; ?>
									...
									<?php if($num==1) echo 'selected'; ?>><?php echo $val['title']; ?></option>
		...
		<div class="col-sm-4 col-lg-3 hidden-xs" id="group-side">
			<ul class="nav nav-pills nav-stacked text-center mb-20">
				<li class="active"><a href="<?php echo U('post/group/index'); ?>">全部主题</a></li>
