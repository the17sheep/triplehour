...
<div class="container">
	<ol class="breadcrumb mb-20 mt-20" id="baobei-term-breadcrumb">
		...
		<li class="active hidden-xs">
			...
			<a href="<?php echo U('Article/Index/pending'); ?>"
				class="publish">待审核文章(<?php echo $countpending;?>)</a><a
				href="<?php echo U('publish/index/add_article'); ?>" class="publish">管理员写文章</a>
				<?php else :?>
				<a href="<?php echo U('publish/index/add_user_article'); ?>"
				class="publish">写文章</a>
				<?php endif; ?>
			</div>
	</ol>
		<?php
		$condition['type'] = 'term_article';
		$terms_article = M('page')->where($condition)->order('id desc')->select();
		if($terms_article)
		:
			?>
		...
		<?php foreach($terms_article as $val) : ?>
			...
			href="<?php echo U('article/index/term?id='.$val['id']); ?>">
					<?php echo $val['title']; ?>
				</a></li>
		<?php endforeach; ?>
		</ul>
		...
		<div class="row">
				...
				<div class="col-md-6">
				<div class="thumbnail">
					<a href="<?php echo mc_get_url($val['id']); ?>" class="img-div"><img
