        ...
        if (is_numeric($id))
        {
            if (mc_user_id() != $id)
            {
                $args_id = M('meta')->where(
                        "meta_key='author' AND meta_value='$id'")->getField(
                        'page_id', true);
                $condition['id'] = array(
                        'in',
                        $args_id
            ...
            {
                // 我关注的
                $args_id1 = M('action')->where(
                        "action_key='perform' AND action_value='guanzhu' AND user_id = '$id'")->getField(
                        'page_id', true);
                // @我的
                $args_id2 = M('action')->where(
                        "action_key='at' AND page_id = '$id'")->getField(
                        'page_id', true);
                // 我的文章发生的
                $args_id3 = M('meta')->where(
                        "meta_key='author' AND meta_value='$id'")->getField(
                        'page_id', true);
                // 最终
                $my_id = array(
                ...
                ;
                $condition1['user_id'] = array(
                        'in',
                        $args_id
                );
                $condition1['page_id'] = array(
                        'in',
                        $args_id5
                );
                ...
                $condition['action_key'] = array(
                        'in',
                        $args_id4
                );
                $condition['_complex'] = $condition1;
                $this->page = M('action')->where($condition)
                    ->order('id desc')
                    ->page($page, mc_option('page_size'))
                    ->select();
    ...
    public function edit($id)
    {
        if (is_numeric($id))
        {
            if (mc_user_id() == $id)
            {
                ...
                {//先暂时不用昵称--pope
//                     $title = M('page')->where(
//                             "title='" . mc_magic_in($_POST['title']) .
//                                      "' AND type ='user'")->getField('id');
...
//                     else
//                     {
//                         mc_update_page(mc_user_id(), 
//                                 mc_magic_in($_POST['title']), 'title');
//                     }
//                     ;
                    if ($_POST['content'])
                    {
                        mc_update_page(mc_user_id(), 
                                mc_magic_in($_POST['content']), 'content');
                    }
                    ;
                    if ($_POST['user_avatar'])
                    {
                        if (mc_get_meta(mc_user_id(), 'user_avatar', true, 
                                'user'))
                        {
                            mc_update_meta(mc_user_id(), 'user_avatar', 
                                    mc_magic_in($_POST['user_avatar']), 'user');
                        }
                        else
                        {
                            mc_add_meta(mc_user_id(), 'user_avatar', 
                                    mc_magic_in($_POST['user_avatar']), 'user');
                        }
                    }
                    ;
                    ...
                    if ($_POST['buyer_name'])
                    {
                        mc_add_meta($id, 'buyer_name', 
                                mc_magic_in($_POST['buyer_name']), 'user');
                    }
                    ;
                    mc_delete_meta($id, 'buyer_province', 'user');
                    if ($_POST['buyer_province'])
                    {
                        mc_add_meta($id, 'buyer_province', 
                                mc_magic_in($_POST['buyer_province']), 'user');
                    }
                    ;
                    mc_delete_meta($id, 'buyer_city', 'user');
                    if ($_POST['buyer_city'])
                    {
                        mc_add_meta($id, 'buyer_city', 
                                mc_magic_in($_POST['buyer_city']), 'user');
                    }
                    ;
                    mc_delete_meta($id, 'buyer_address', 'user');
                    if ($_POST['buyer_address'])
                    {
                        mc_add_meta($id, 'buyer_address', 
                                mc_magic_in($_POST['buyer_address']), 'user');
                    }
                    ;
                    mc_delete_meta($id, 'buyer_phone', 'user');
                    if ($_POST['buyer_phone'])
                    {
                        mc_add_meta($id, 'buyer_phone', 
                                mc_magic_in($_POST['buyer_phone']), 'user');
                    }
                    ;
                    if ($_POST['user_email'])
                    {
                        mc_update_meta(mc_user_id(), 'user_email', 
                                mc_magic_in($_POST['user_email']), 'user');
                    }
                    else
                    {
                        $this->error('邮箱必须填写！');
                    ...
                    if (I('param.pass'))
                    {
                        ...
                        {//这里不能用I 函数得到密码得到的密码加密后有问题。--pope
                            mc_update_meta(mc_user_id(), 'user_pass', 
                                    md5($_POST['pass'] . mc_option('site_key')), 
                                    'user');
    ...
    public function site_control()
    {
        if (mc_is_admin())
        {
            if ($_POST['home_control'])
            {
                ...
                mc_update_option('home_description', 
                        I('param.home_description'));
            ...
            elseif ($_POST['group_control'])
            {
                ...
                mc_update_option('group_description', 
                        I('param.group_description'));
            ...
            elseif ($_POST['article_control'])
            {
                ...
                mc_update_option('article_keywords', 
                        I('param.article_keywords'));
                mc_update_option('article_description', 
                        I('param.article_description'));
    ...
    public function shoucang($id)
    {
        if (is_numeric($id))
        {
            $args_id = M('action')->where(
                    "user_id='$id' AND action_key='perform' AND action_value='shoucang'")->getField(
                    'page_id', true);
            $condition['id'] = array(
                    'in',
                    $args_id
    ...
    public function pro($id, $page = 1)
    {
        if (is_numeric($id))
        {
            if (mc_user_id() == $id)
            {
                $this->page = M('action')->where(
                        "user_id='" . mc_user_id() .
                                 "' AND action_key IN ('trade_wait_send','trade_wait_cofirm','trade_wait_finished','trade_wait_hdfk')")
                    ->order('id desc')
                    ->page($page, mc_option('page_size'))
                    ->select();
                $count = M('action')->where(
                        "user_id='" . mc_user_id() .
                                 "' AND action_key IN ('trade_wait_send','trade_wait_cofirm','trade_wait_finished','trade_wait_hdfk')")->count();
                $this->assign('id', $id);
                $this->assign('count', $count);
                $this->assign('page_now', $page);
    ...
    public function fans($id, $page = 1)
    {
        if (is_numeric($page))
        {
            $condition['type'] = 'user';
            $args_id = M('action')->where(
                    "action_key='perform' AND action_value='guanzhu' AND page_id='$id'")->getField(
                    'user_id', true);
            $condition['id'] = array(
                    'in',
                    $args_id
