<?php
// --------------- PUBLIC ---------------//
// 遍历输出(引用$filelist)为数组，文件夹下所有文件--pope
function list_dir($dir, &$filelist)
{
	if(is_dir($dir))
	{
		if($dh = opendir($dir))
		{
			while(($file = readdir($dh)) !== false)
			{
				...
				{
					// echo "<b><font color='red'>文件名：</font></b>",$dir . "/" .
					// $file, "<br><hr>";
					list_dir($dir . "/" . $file, $filelist);
				}
				else
				{
					if($file != "." && $file != "..")
...
// 验证字符串长度--pope
function validate_string_len($string, $minLength, $maxLength)
{
...
function mc_title()
{
	...
	elseif(MODULE_NAME == 'Publish')
	{
		if(ACTION_NAME == 'index')
		{
			$title = '发布项目 - ' . mc_option('site_name');
		}
	...
	elseif(MODULE_NAME == 'Pro')
	{
		if(CONTROLLER_NAME == 'Cart')
		{
			if(ACTION_NAME == 'index')
			{
				$title = '我的合伙项目 - ' . mc_option('site_name');
			}
		...
		elseif(ACTION_NAME == 'index')
		{
			...
			{
				$title = '全部项目 - ' . mc_option('site_name');
			}
		}
	...
	elseif(MODULE_NAME == 'User')
	{
		...
		elseif(CONTROLLER_NAME == 'Index')
		{
			...
			{
				$title = $display_name . '的合伙项目 - ' . mc_option('site_name');
			}
	...
	elseif(MODULE_NAME == 'Control')
	{
		if(CONTROLLER_NAME == 'Index')
		{
			...
			{
				$title = '项目管理 - ' . mc_option('site_name');
			}
...
function mc_get_url($page_id)
{
	...
	:
		if(C('URL_MODEL') == 2)
		{
			$permalink = mc_site_url() . '/pro-user-' . $page_id . '.html';
		}
		else
		{
			$permalink = U('pro/index/single_user?id=' . $page_id);
		}
	...
	elseif($type == 'article_pending')
	:
		if(C('URL_MODEL') == 2)
		{
			$permalink = mc_site_url() . '/article-user-' . $page_id . '.html';
		}
		else
		{
			$permalink = U('article/index/single_user?id=' . $page_id);
		}
...
function mc_is_mobile()
{
	...
	elseif(strpos($_SERVER['HTTP_USER_AGENT'], 'Mobile') !== false || // many
	                                                                  // mobile
	                                                                  // devices
	                                                                  // (all
	                                                                  // iPhone,
...
require_once ('go.php');
// require_once (APP_PATH . 'function.php');删除了APP_PATH下的function.php文件这里也注释掉--pope
